...
import { Container, FormControl, Input, Button, Image, InputGroup, Textarea, InputRightElement, Flex, Box } from '@chakra-ui/react';
...
export const Native = observer(() => {
  const { god, token, lang } = useStore();

  const store = useLocalStore(() => ({
    amount: new BigNumberInputState({}),
    receiverAddress: new StringState(),
    ...
    get state() {
      ...
      const valid = store.curToken && store.amount.value && store.receiverAddress.value;
      return {
        valid,
    ...
    async onSubmit() {
      ...
      const [err, res] = await helper.promise.runAsync(store.curToken.transfer({ params: [store.receiverAddress.value, store.amount.value.toFixed(0, 1)] }));
      ...
      <form>
        <FormControl mt={20}>
          ...
          <Box border="1px solid" borderRadius="md" borderColor="inherit" mt={4}>
            ...
            <InputGroup>
              <Input border="none" placeholder={god.currentNetwork.info.token.tokenExample} value={store.receiverAddress.value} onChange={(e) => store.receiverAddress.setValue(e.target.value)} />
            </InputGroup>
          </Box>
